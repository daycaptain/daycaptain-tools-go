...
TDA_VERSION 			?= $(shell git describe --tags --always --dirty)
TDA_OUTPUT        ?= tda
TDA_REVISION      := $(shell git describe --tags --always --dirty)
DATE              := $(shell date -u -d "@$(SOURCE_DATE_EPOCH)" '+%FT%T%z' 2>/dev/null || date -u '+%FT%T%z')
BUILDFLAGS_NOPIE  := -trimpath -ldflags="-s -w -X main.version=$(TDA_VERSION) -X main.commit=$(TDA_REVISION) -X main.date=$(DATE)" -gcflags="-trimpath=$(GOPATH)" -asmflags="-trimpath=$(GOPATH)"
...
$(TDA_OUTPUT): $(GOFILES_BUILD)
	@echo -n ">> BUILD, version = $(TDA_VERSION)/$(TDA_REVISION), output = $@"
